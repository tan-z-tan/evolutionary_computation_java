...
public class WallFollowingEvolutionModel extends EvolutionModel<Robot, GpEnvironment<Robot>>
{
  ...
  public WallFollowingEvolutionModel(GpEnvironment<Robot> environment)
  {
    super(environment);
  }
  ...
  public void updateGeneration()
  {
    ...
    // RouletteSelector<Robot>(_environment.getPopulation());

    int phenotypeNum = _environment.getPopulationSize();
    List<Robot> newRobots = new ArrayList<Robot>(phenotypeNum);
    for (int i = 0; i < _environment.getEliteSize(); i++)
    {
      Robot elite = createRobot();
      elite.setGene((GpNode) bestIndividual.getRootNode().clone());
      newRobots.add(elite);
    }
    for (; newRobots.size() < phenotypeNum;)
    {
      if (newRobots.size() < (phenotypeNum) * _environment.getCrossoverRatio())
      {
        GpIndividual parentA = (GpIndividual) selector.getRandomPType();
        GpIndividual parentB = (GpIndividual) selector.getRandomPType();
        while (parentB == parentA)
        {
          parentB = (GpIndividual) selector.getRandomPType();
        }

        GpNode[] childrenTree = GpTreeManager.crossover(parentA.getRootNode(), parentB.getRootNode(), _environment);
        Robot childA = createRobot();
        Robot childB = createRobot();
        childA.setRootNode(childrenTree[0]);
        childB.setRootNode(childrenTree[1]);
        newRobots.add(childA);
        if (newRobots.size() + 1 < phenotypeNum)
        {
          newRobots.add(childB);
        }
      } else
      {
        Robot child = createRobot();
        child.setRootNode(selector.getRandomPType().getRootNode());
        newRobots.add(child);
      }
    }
    for (Robot robot : newRobots)
    {
      if (_environment.getMutationRatio() > RandomManager.getRandom())
      {
        GpNode mutatedRoot = GpTreeManager.mutation(robot.getRootNode(), _environment);
        robot.setRootNode(mutatedRoot);
      }
    }
    _field.setRobots(newRobots);
    _environment.setPopulation(newRobots);
  }
